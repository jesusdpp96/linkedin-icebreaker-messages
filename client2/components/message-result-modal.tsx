...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter, DialogClose } from "@/components/ui/modal"
...
export function MessageResultModal({ isOpen, onClose, isLoading, messages }: MessageResultModalProps) {
      ...
      >
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-lg text-zinc-300">Estamos generando tus mensajes, por favor espera...</p>
          </div>
        ) : (
                  ...
                  <div className="flex flex-col space-y-4">
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <h3 className="mt-2 text-lg font-bold text-white">{message.templateTitle}</h3>
                      </div>
                      <Button
                        onClick={() => copyToClipboard(message.message, index)}
                        className="bg-linkedin hover:bg-linkedin-light text-white border-0"
                        size="sm"
                      >
                        {copiedIndex === index ? (
                          <>
                    ...
                    <div className="bg-zinc-900 rounded-lg p-4 border border-zinc-700">
                      ...
                      <div className="flex items-center mb-4">
                        <div className="relative h-12 w-12 rounded-full overflow-hidden mr-3">
                          <Image
                            src={message.receiverProfilePicture || "/placeholder.svg"}
                        ...
                        <div>
                          <h4 className="font-semibold text-white">{message.receiverName}</h4>
                          <p className="text-xs text-zinc-400 line-clamp-1">{message.receiverHeadline}</p>
                        </div>
                      </div>

                      {/* Message Content */}
                      <div className="bg-zinc-800 rounded-lg p-4 mb-4">
                        <p className="text-zinc-200">{message.message}</p>
                      </div>
                      ...
                      <div className="flex items-center">
                        ...
                        <div>
                          <h4 className="font-semibold text-white text-sm">{message.senderName}</h4>
                          <p className="text-xs text-zinc-400 line-clamp-1">{message.senderHeadline}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-linkedin-dark/20 border border-linkedin/30 rounded-lg p-4">
                      <h4 className="font-semibold text-linkedin-light mb-2">Instrucción:</h4>
                      <p className="text-sm text-zinc-300">{message.instruction}</p>
                    </div>

                    {/* Source Posts */}
                    {message.sourcePosts && message.sourcePosts.length > 0 && (
                      <div className="bg-zinc-800/50 rounded-lg p-4">
                        <h4 className="font-semibold text-zinc-300 mb-2">Posts relacionados:</h4>
        ...
        <DialogFooter>
          {!isLoading && (
            <DialogClose asChild>
              <Button variant="outline" className="border-zinc-700 text-white hover:bg-zinc-800" onClick={onClose}>
                Cerrar
              </Button>
            </DialogClose>
          )}
